RSpec.describe RavenDB::PutIndexesOperation, database: true do
  it "puts index with success" do
    @_index = RavenDB::IndexDefinition.new(name: "region", index_map:)
  ...
  it "gets index with success" do
    @_index = RavenDB::IndexDefinition.new(name: "get_index", index_map:)
    store.operations.send(RavenDB::PutIndexesOperation.new(@_index))
  ...
  it "deletes index with success" do
    @_index = RavenDB::IndexDefinition.new(name: "delete", index_map:)
    store.operations.send(RavenDB::PutIndexesOperation.new(@_index))
